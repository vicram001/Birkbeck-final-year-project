...
# download NLTK resources (run once), and load required tools.
# downloads the tokenizer model for splitting text into words.
nltk.download("punkt")
...
# multiple texts and labels (>= 2 classes), prepare example texts and labels
# raw example sentences
texts = [
...
# vectorize, convert text to numbers with TF-IDF features
# Creates numerical features based on term frequency–inverse document frequency. Each document is now represented as a sparse vector of numbers.
vectorizer = TfidfVectorizer()
X_train = vectorizer.fit_transform(X_train_texts)
X_test  = vectorizer.transform(X_test_texts)

# train the Logistic Regression classifier
# Learns a statistical model to predict the class label from the numeric features.
...
# predict a new text
# In the minimal working example, splits the data into train/test sets so that the model can be evaluated on unseen examples.
